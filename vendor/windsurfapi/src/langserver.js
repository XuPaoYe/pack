...
export async function ensureLs(proxy = null) {
  ...
  const promise = (async () => {
    ...
    let port = isDefault ? _defaultPort : _nextPort++;
    ...
    if (!isDefault) {
      let tries = 0;
      while (await isPortInUse(port)) {
        if (++tries > 50) throw new Error(`No free port for LS in range starting ${_defaultPort + 1}`);
...
export function getLsPort() {
  return _pool.get('default')?.port || _defaultPort;
}
...
export async function startLanguageServer(opts = {}) {
  ...
  const requestedPort = Number.parseInt(String(opts.port || process.env.LS_PORT || DEFAULT_PORT), 10);
  if (Number.isFinite(requestedPort) && requestedPort > 0 && requestedPort <= 65535) {
    _defaultPort = requestedPort;
