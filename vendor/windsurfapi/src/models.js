...
export const MODELS = {
  ...
  // Claude Opus 4.7 — effort-tiered family (Low / Medium / High / XHigh / Max)
  // Max is the flagship for heaviest work at 30 credits/request.
...
// Claude Opus 4.7 — dash-form aliases so both `claude-opus-4.7-max` and
// `claude-opus-4-7-max` resolve. Also map friendly labels "opus-4.7-max" etc.
_lookup.set('claude-opus-4-7-low',    'claude-opus-4.7-low');
_lookup.set('claude-opus-4-7-medium', 'claude-opus-4.7-medium');
...
_lookup.set('gpt-5-2-medium', 'gpt-5.2');
_lookup.set('gpt-5.5', 'gpt-5.5-medium');
_lookup.set('gpt-5-5', 'gpt-5.5-medium');
...
export function resolveModelWithOptions(name, options = {}) {
  ...
  if (/^gpt-5\.5/.test(model) || /^gpt-5-5/.test(String(raw || ''))) {
    return resolveVariant('gpt-5.5', effort || 'medium', false);
  }
