...
fn default_app_settings() -> AppSettings {
    AppSettings {
        ...
        api_service_default_model: DEFAULT_API_SERVICE_MODEL.to_string(),
    }
}
...
fn effective_api_service_model(model: &str) -> String {
    ...
    if model.is_empty() {
        DEFAULT_API_SERVICE_MODEL.to_string()
    } else {
        model.to_string()
    }
}
...
fn init_app_db(conn: &Connection) -> Result<(), String> {
    ...
    conn.execute(
        "UPDATE accounts SET provider = 'superai' WHERE provider = 'superai'",
        [],
    )
...
fn persist_existing_account_exact(
    ...
    let stored_email = account.email.clone();
    let stored_display_name = account.display_name.clone();
    let stored_provider = account.provider.clone();
    let updated = conn
        .execute(
            r#"
            UPDATE accounts
...
fn parse_stored_account_json(account_json: &str) -> Result<ManagedAccount, String> {
    ...
    let is_encrypted_local_credential_wrapper = value
        .get("encrypted")
        .and_then(Value::as_bool)
        .unwrap_or(false)
        && matches!(value.get("provider").and_then(Value::as_str), Some("superai"));
    if is_encrypted_local_credential_wrapper {
        let payload = value
            .get("payload")
            .and_then(Value::as_str)
            .ok_or_else(|| "SuperAI 加密账号记录缺少 payload".to_string())?;
        let decrypted = decrypt_local_credential_text(payload)
...
fn upsert_account(conn: &Connection, account: &ManagedAccount) -> Result<(), String> {
    ...
    let stored_email = account_to_write.email.clone();
    let stored_display_name = account_to_write.display_name.clone();
    let stored_provider = account_to_write.provider.clone();
    conn.execute(
        r#"
...
fn legacy_local_credential_aes_key_iv() -> Result<([u8; 32], [u8; 16]), String> {
    let key = hex::decode(LEGACY_PROVIDER_AES_KEY_HEX)
        .map_err(|error| format!("解析 SuperAI AES key 失败: {error}"))?;
    let iv = hex::decode(LEGACY_PROVIDER_AES_IV_HEX)
...
fn local_credential_master_key_backup_path() -> Result<PathBuf, String> {
    Ok(app_storage_root()?.join("superai_master_key.bak"))
}

fn local_credential_master_key_path() -> Result<PathBuf, String> {
    Ok(app_storage_root()?.join("superai_master_key"))
}

fn load_existing_local_credential_master_key() -> Result<[u8; 32], String> {
    let path = local_credential_master_key_path()?;
    let backup_path = local_credential_master_key_backup_path()?;
...
fn decrypt_local_credential_text_v1(cipher_text: &str) -> Result<String, String> {
    type Aes256CbcDec = cbc::Decryptor<Aes256>;
    let (key, iv) = legacy_local_credential_aes_key_iv()?;
...
fn decrypt_local_credential_text(cipher_text: &str) -> Result<String, String> {
    type Aes256CbcDec = cbc::Decryptor<Aes256>;
    let raw = cipher_text.trim();
    if let Some(rest) = raw.strip_prefix(LOCAL_CREDENTIAL_CRYPTO_V2_PREFIX) {
        let (iv_b64, cipher_b64) = rest
            .split_once(':')
            .ok_or_else(|| "SuperAI v2 密文格式非法".to_string())?;
        let key = load_existing_local_credential_master_key()?;
    ...
    decrypt_local_credential_text_v1(raw)
}
